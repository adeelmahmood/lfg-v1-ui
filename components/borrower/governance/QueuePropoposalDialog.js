...
export default function QueuePropoposalDialog({ loanProposal, onQueuedSuccess }) {
    const [queueModalOpen, setQueueModalOpen] = useState();
    ...
    const governorFunctionName = "queue";
    ...
    const { isLoading: isTxLoading, isSuccess: isSuccess } = useWaitForTransaction({
        hash: data?.hash,
        onSuccess(data) {
            const abi = ["event ProposalQueued(uint256 proposalId, uint256 eta)"];
            findEvent(
                abi,
    ...
    return (
        <>
            ...
            >
                <QueueListIcon className="inline h-6 fill-current align-top text-white dark:text-gray-800" />
                <span className="ml-2 hidden md:inline">Queue this Proposal</span>
            </button>

            <DialogComponent
                heading="Queue Proposal"
                isModelOpen={queueModalOpen}
